...
| TH-009 | Replay equivalence under same profile must be observational | Conformance pack | PACK.replay.appliance | promoted | Replay bundle and validation doctrine now promoted; executable conformance remains required |
...
| TH-013 | Replay bundles must preserve source identity, schema version, and capture-loss status explicitly | Conformance pack + schema validation | PACK.replay.appliance | promoted | Bound by Replay doctrine and `CONSTR-029` |
| TH-014 | Canonical forensic traces must support causality queries and differential triage | Conformance pack | PACK.trace.forensic_plane; PACK.diff.cross_engine.continuous | promoted | Replay explain and forensic surfaces are now doctrinally required |
| TH-015 | Witness distillation must be predicate-driven, replay-closed, and explicit on unstable or insufficient capture | Conformance pack + retained witness evidence | PACK.replay.appliance; PACK.diff.cross_engine.continuous | promoted | Bound by Replay doctrine and `CONSTR-033` |
| TH-016 | Adapter capability claims must be machine-readable and validated before downstream reliance | Conformance pack + manifest validation | PACK.replay.appliance | promoted | Capability manifests and `C0`..`C5` ladder are now doctrinal |
| TH-017 | Witness lifecycle and quarantine state must gate pack promotion and retention | Conformance pack + lifecycle evidence | PACK.replay.appliance; PACK.diff.cross_engine.continuous | promoted | Quarantined and explanatory-only witnesses are not pack-eligible |
| TH-018 | Full-model optimization must consume OxFunc-owned semantics through resolved call-site handles and metadata, not evaluator-local function special cases | Conformance pack + metadata contract | PACK.fec.oxfunc_metadata_contract; PACK.optimization.semantic_equivalence | promoted | W096 direction; supports graph/backend optimization while preserving profile-defined worksheet-visible semantics |
