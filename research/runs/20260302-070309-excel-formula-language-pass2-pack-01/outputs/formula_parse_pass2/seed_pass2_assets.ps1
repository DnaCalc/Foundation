...
function Get-ManualPrepNote {
    ...
    switch ($Profile) {
        ...
        'manual_name_shadowing' {
            return 'Add a sheet-local name MyName on Sheet1 that intentionally shadows workbook-level MyName before execution.'
        }
        'external_workbook_present' {
            return 'Create support workbook Book2.xlsx (Sheet1!A1 populated) under fixtures/formula_parse_pass2/support before execution.'
        }
        default {
            return ''
        }
...
function Get-FixtureProfile {
    ...
    if ($req.Contains('sheet-local myname')) { return 'manual_name_shadowing' }
...
function New-SheetWrites {
    ...
    if ($Profile -eq 'workbook_name' -or $Profile -eq 'manual_name_shadowing') {
        $writes += [ordered]@{
            kind = 'name'
            address = ''
            value = [ordered]@{
...
foreach ($seed in $seedRows) {
    ...
    $operations = New-Operations -Profile $fixtureProfile -TargetCell $targetCell -FormulaInput $formulaInput -ExpectedOutcomeClass $seed.expected_outcome_class
...
$readmeLines = @(
    ...
    'Linked-data and some external-reference/name-shadowing branches remain explicitly tracked as unresolved harness lanes.'
)
