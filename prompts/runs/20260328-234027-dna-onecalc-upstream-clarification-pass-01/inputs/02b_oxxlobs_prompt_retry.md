...
Goal:
Do a narrower docs-first pass that creates or tightens the canonical OneCalc-facing observation-consumer contract for `OxXlObs`. Do not change code. Only touch docs needed to make the downstream comparison baseline and observation-envelope story clear.

Read only these first:
1. `.\README.md`
2. `.\CHARTER.md`
3. `.\docs\spec\README.md`
4. `.\docs\spec\OXXLOBS_SCOPE_AND_BOUNDARY.md`
5. `.\docs\spec\OXXLOBS_ARCHITECTURE_AND_CAPTURE_MODEL.md`
6. `.\docs\spec\OXXLOBS_BUNDLE_EMISSION_AND_HANDOFF_MODEL.md`
7. `.\docs\spec\OXXLOBS_SCENARIO_REGISTER.md`
...
Preferred output:
1. add one narrow canonical spec note under `docs\spec\` for `DNA OneCalc` as a downstream consumer of `OxXlObs`,
...
Constraints:
1. `OxXlObs` owns observation and retained evidence, not replay doctrine and not Excel semantic truth.
...
Deliverables:
1. updated docs,
2. a short final summary listing changed files, the authoritative `OxXlObs` doc set `DNA OneCalc` should use, and remaining gaps.
